...
|---|---|---|
| A standalone commodity pricing dashboard | Answers an adjacent problem, not the manager's actual stated pain point (information work and logistics, not pricing accuracy). Also the most obvious build, every candidate likely gravitates here, low differentiation. | Pricing is folded in as one input to the packet instead of the centerpiece, less flashy on its own, more accurate to the brief. |
...
| Live MCP connector for pricing inside the prototype itself | A live demo depending on a third-party server responding correctly in the room is unnecessary risk for zero added insight value. | One-time static pull instead; MCP referenced narratively as the production-path architecture, not live in the demo. |
